...
class EventController extends Controller
{
    public function index()
    {
        ...
        foreach ($events as $event) {
            $event->image_url = 'data:image/jpeg;base64,' . base64_encode($event->image_url);
        }
    ...
    {
        $event_detail = DB::table('events')->where('id', $id)->first();

        if ($event_detail) {
            $event_detail->image_url = 'data:image/jpeg;base64,' .base64_encode( $event_detail->image_url);
        } else {
